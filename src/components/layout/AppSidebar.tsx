    ...
    <li className="w-full">
      {item.path ? (
        <Link 
          to={item.path} 
          className={cn(
            'flex items-center py-3 px-4 text-white hover:bg-[#2a314a] w-full transition-all duration-200', 
            {
              'bg-[#2a314a]': isActive,
        ...
        <button 
          className={cn(
            'flex w-full items-center py-3 px-4 text-white hover:bg-[#2a314a] transition-all duration-200', 
            {
              'bg-[#2a314a]': isActive,
              'justify-center': isCollapsed
            }
          ...
          {item.children.map(child => (
            <li key={child.name}>
              <Link 
                to={child.path} 
                className={cn(
                  'flex items-center py-2 px-11 text-sm text-white/80 hover:bg-[#2a314a] hover:text-white transition-all duration-200', 
                  {
          ...
          <div className={cn("flex items-center h-16 px-4 border-b border-white/10", {
            ...
            <div className={cn("flex items-center", {
              ...
              {isCollapsed ? (
                ...
                <img 
                  src="/lovable-uploads/f8142104-f549-4b88-961d-3a414132f32c.png" 
                  alt="EPOMS Logo" 
                  className="h-8" 
                />
              )}
            ...
            {!isMobile && (
              <button 
                onClick={() => setIsCollapsed(!isCollapsed)} 
                className={cn(
                  "ml-auto p-1 rounded-md text-white hover:bg-[#2a314a] focus:outline-none transition-all duration-200", 
                  {
