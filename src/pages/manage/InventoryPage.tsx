...
const InventoryPage: React.FC<InventoryPageProps> = ({
  ...
  const columns: Column[] = [{
    ...
    cell: value => <span>RM {value.toFixed(2)}</span>
  }, {
    id: 'totalPrice',
    header: 'Total Price',
  ...
  return <div className="space-y-6">
      {!hideHeader && <PageHeader title="Inventory Management" subtitle="Manage spare parts inventory" icon={<Package className="h-6 w-6" />} onSearch={handleSearch} addNewLabel="+ Add New Item" onAddNew={() => navigate('/manage/inventory/new')} actions={<div className="flex space-x-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title="Total Items" value={totalItems} icon={<Package className="h-5 w-5" />} changeLabel="Listed spare parts" />

        <KpiCard title="Total Inventory Value" value={`RM ${totalInventoryValue.toLocaleString('en-MY', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      })}`} icon={<Package className="h-5 w-5" />} changeLabel="Sum of all items" />

        <KpiCard title="Low Stock Alerts" value={`${lowStockItemsCount} items low`} icon={<AlertTriangle className="h-5 w-5 text-amber-500" />} className="relative">
          <div className="mt-2 text-xs text-gray-500 space-y-1">
            {lowStockItems.map(item => <div key={item.id} className="flex items-center justify-between">
                <div>{item.itemName}: {item.balance} units</div>
                <Button variant="ghost" size="sm" className="h-6 p-0 text-blue-600 hover:text-blue-800" onClick={() => handleOpenLowStockModal(item)}>
        ...
        <KpiCard title="Stores" value={`${totalStores} Stores`} icon={<Warehouse className="h-5 w-5" />} className="relative">
      ...
      <Card>
        <CardContent className="p-6">
          <DataTable columns={columns} data={inventory} onRowClick={handleRowClick} />
        </CardContent>
        <CardFooter className="flex justify-between items-center border-t p-4">
          {/* Existing Download/Export buttons would go here */}
          <div className="w-full flex flex-wrap gap-2 sm:flex-nowrap">
            <Button variant="outline" size="lg" onClick={handleTrackAllPOs} className="gap-2 flex-1 justify-center text-base font-medium">
              <ListChecks className="h-5 w-5" /> Track POs
            </Button>
            <Button size="lg" onClick={handleCreatePO} className="gap-2 flex-1 justify-center text-base font-medium">
      ...
      <LowStockAlertModal isOpen={isLowStockModalOpen} onClose={() => setIsLowStockModalOpen(false)} item={selectedItem} />

      {/* Request PO Modal */}
      <RequestPOModal isOpen={isRequestPOModalOpen} onClose={() => setIsRequestPOModalOpen(false)} item={selectedItem} />

      {/* Purchase Order Tracker Modal */}
      <PurchaseOrderTracker isOpen={isPOTrackerModalOpen} onClose={() => setIsPOTrackerModalOpen(false)} selectedItem={selectedItem?.itemName} />
    </div>;
};
